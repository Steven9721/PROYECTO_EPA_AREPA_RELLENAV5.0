document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderFavorites() {
        favoritesContainer.innerHTML = ''; // Limpiar el contenedor antes de renderizar
        ...
        favorites.forEach(productId => {
            ...
            if (product) {
                ...
                favoriteCard.setAttribute('data-id', productId); // Añadir un atributo data-id para identificar el producto
    ...
    // Llamar a la función para renderizar los productos favoritos
    renderFavorites();
    ...
    favoritesContainer.addEventListener('click', function(event) {
        if (event.target.classList.contains('btn-remove')) {
            ...
            // Filtrar el producto eliminado de la lista de favoritos
            favorites = favorites.filter(id => id !== productId);
            
            // Actualizar el almacenamiento local
            localStorage.setItem('favorites', JSON.stringify(favorites));
            
            // Volver a renderizar los favoritos
            renderFavorites();
        }
    });
    ...
    // Obtener el elemento del mensaje "No hay favoritos"
    const noFavoritesMessage = document.getElementById('no-favorites');

    // Función para mostrar u ocultar el mensaje "No hay favoritos"
    function toggleNoFavoritesMessage() {
        if (favorites.length === 0) {
            noFavoritesMessage.style.display = 'block'; // Mostrar el mensaje si no hay favoritos
        } else {
            noFavoritesMessage.style.display = 'none'; // Ocultar el mensaje si hay productos
        }
    }

    // Llamar a la función para verificar al inicio si hay favoritos
    toggleNoFavoritesMessage();

    // Escuchar el clic en el botón de eliminar dentro del contenedor de favoritos
    favoritesContainer.addEventListener('click', function(event) {
        if (event.target.classList.contains('btn-remove')) {
            // Eliminar el producto de favoritos ya está en tu código original

            // Después de eliminar el producto, actualizar la visibilidad del mensaje
            toggleNoFavoritesMessage();
        }
    });
